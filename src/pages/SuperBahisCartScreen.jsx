...
import SuperBahisCartItemComponent from '../components/SuperBahisCartItemComponent';
...
export default function () {
  ...
  const handleOrder = () => {
    const destinationScreen = cart.length
      ? 'SuperBahisCartSuccessScreen'
      : 'SuperBahisHomeScreen';
        ...
        <>
          <View style={{height: height * 0.7}}>
            <ScrollView style={styles.flex} contentContainerStyle={styles.main}>
              {cart.map((item, index) => (
                <SuperBahisCartItemComponent item={item} key={index} />
              ))}
            </ScrollView>
          </View>
        </>
